...
public class SerialDataManager {
    ...
    public static void parseData(String data, SerialListener serialListener) {
        if (TextUtils.isEmpty(data) || serialListener == null)
            return;
        Log.e("SerialDataManager", "接收到串口数据:" + data);
        try {
            ...
            switch (type) {
                ...
                case 5:
                    if (jsonObject.has("var2")) {
                        v2 = jsonObject.getString("var2");
                    }

                    serialListener.onError(var1, v2);
                    break;
                case 7:
                    if (jsonObject.has("var2")) {
                        v2 = jsonObject.getString("var2");
                    }
                    serialListener.on433(var1, v2);//433 数据
                    break;
                default:
                    if (serialListener instanceof TestSerialListener) {
