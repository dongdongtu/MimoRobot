package com.chance.mimorobot.widget.neteasedisc.widget;
...
//        int pivotY = (int) (DisplayUtil.SCALE_NEEDLE_PIVOT_Y * mScreenWidth);
        ...
        mIvNeedle.setPivotX(0);
        mIvNeedle.setPivotY(0);
        mIvNeedle.setRotation(DisplayUtil.ROTATION_INIT_NEEDLE);
        mIvNeedle.setImageBitmap(originBitmap);
     ...
     */
    private Drawable getDiscDrawable(Bitmap musicPicRes) {
        int discSize = (int) (mScreenWidth * DisplayUtil.SCALE_DISC_SIZE);
    ...
    private Bitmap getMusicPicBitmap(int musicPicSize, Bitmap musicPicRes) {
...
//                musicPicRes, options), musicPicSize, musicPicSize, true);
        return Bitmap.createScaledBitmap(musicPicRes, musicPicSize, musicPicSize, true);
    }
    ...
    public void setMusicDataList(List<MusicData> musicDataList) {
        ...
        for (MusicData musicData : mMusicDatas) {
            ...
            final View discLayout = LayoutInflater.from(getContext()).inflate(R.layout.layout_disc,
                    mVpContain, false);
            final ImageView disc = (ImageView) discLayout.findViewById(R.id.ivDisc);

            Glide.with(getContext())
                    .load(R.drawable.ic_muisc2)
                    .asBitmap()   //强制转换Bitmap
                    .into(new SimpleTarget<Bitmap>() {
                        ...
                        public void onResourceReady(final Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                            new Thread(new Runnable() {
                                @Override
                                public void run() {
                                    final Drawable drawable = getDiscDrawable(resource);
